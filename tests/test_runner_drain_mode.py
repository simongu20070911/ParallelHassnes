...
def _write_harness_config(*, runs_root: Path, runner_ids: list[str]) -> None:
    ...
    (runs_root / "_system" / "harness_config.json").write_text(
        json.dumps(
            {
                "harness_config_version": "hc_drain",
...
class RunnerDrainModeTests(unittest.TestCase):
    def test_drained_runner_does_not_receive_new_attempts(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            runner_a = "runner_a"
            runner_b = "runner_b"
            _write_harness_config(runs_root=runs_root, runner_ids=[runner_a, runner_b])
            ...
            # Mark runner_a as drained (not accepting new work).
            (runners_root / runner_a).mkdir(parents=True, exist_ok=True)
            (runners_root / runner_a / "drain.json").write_text(json.dumps({"drain": True}) + "\n", encoding="utf-8")
            ...
            attempt_dirs = list(store.paths.attempts_dir(batch_id, "j1", "s1").glob("*"))
            self.assertEqual(len(attempt_dirs), 1)
            meta = store.read_attempt_meta(attempt_dirs[0]) or {}
            self.assertEqual(meta.get("runner_id"), runner_b)
